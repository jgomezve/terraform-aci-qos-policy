...
resource "aci_rest_managed" "qosDscpClass" {
  ...
  dn         = "${aci_rest_managed.qosCustomPol.id}/dcsp-${each.value.dscp_from}-${each.value.dscp_to != null ? each.value.dscp_to : each.value.dscp_from}"
...
resource "aci_rest_managed" "qosDot1PClass" {
  ...
  dn         = "${aci_rest_managed.qosCustomPol.id}/dot1P-${each.value.dot1p_from}-${each.value.dot1p_to != null ? each.value.dot1p_to : each.value.dot1p_from}"
